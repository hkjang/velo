...
│                                             │
│ [공통 핸들러]                                │
│  HttpServerCodec(maxInitialLine, maxHeader)  │
│  → HttpObjectAggregator(maxContentLength)   │
│  → ChunkedWriteHandler                      │
│  → IdleStateHandler(idleTimeoutSeconds)     │
│  → IdleChannelHandler (유휴 연결 종료)       │
│  → HttpContentCompressor (gzip, 조건부)     │
│  → NettyHttpChannelHandler                  │
│                                             │
│ [WebSocket 경로]                             │
...
│  /health  → 내장 핸들러                      │
│  /info    → 내장 핸들러                      │
│  /metrics → MetricsCollector 스냅샷 (JSON)   │
...
- In-memory JSESSIONID 세션 + TTL 만료 스케줄러 (설정 가능한 타임아웃) ([상세 문서](session-management.md))
- **WAR 배포**: web.xml 파싱, 클래스로더 격리 (parent-first/child-first) ([상세 문서](war-deployment.md))
- **배포 디렉토리**: `deploy/` 디렉토리 기반 자동 WAR 배포 + `DeploymentRegistry` 관리
- **핫 디플로이**: `WatchService` 기반 WAR 파일 변경 감지 및 자동 재배포 ([상세 문서](war-deployment.md))
- **구조화된 로깅**: 액세스/에러/감사 로그, JSON 형식 ([상세 문서](structured-logging.md))
- **메트릭 수집**: `LongAdder` 기반 요청 수, 활성 연결, 응답 시간, HTTP 상태 코드 분포 ([상세 문서](structured-logging.md))
...
- **TLS 감지**: `isSecure()`, `getScheme()` — SslHandler 기반 자동 감지
...
| 7 | Gzip 압축 + 유휴 연결 타임아웃 + HTTP 코덱 제한 | **완료** |
| 8 | 메트릭 수집 (LongAdder 기반) | **완료** |
| 9 | Multipart 지원 (Part API) | **완료** |
| 10 | 배포 디렉토리 + 핫 디플로이 | **완료** |
| 11 | 세션 타임아웃 설정 연동 | **완료** |
| 12 | TLS 감지 (isSecure/getScheme) | **완료** |
