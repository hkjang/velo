...
MCP Client (Claude, IDE, Agent)
    │
    ├── POST /ai-platform/mcp          → JSON-RPC 2.0 요청/응답
    ├── GET  /ai-platform/mcp (SSE)    → 서버 이벤트 스트림
    └── GET  /ai-platform/mcp/health   → 상태 확인
         │
    ┌────┴────────────────────────┐
    │   McpServer (JSON-RPC)     │
    │   ├── ToolRegistry          │
...
서버 시작 후 로그에서 확인:
```
MCP server installed at /ai-platform/mcp (admin at /ai-platform/mcp/admin/*)
```
...
6. (선택) Client → GET /ai-platform/mcp (SSE)           → 서버 이벤트 수신
```
...
모든 Admin API는 `/ai-platform/mcp/admin/*` 경로에 있습니다.
...
#### 등록된 툴 조회
```bash
curl http://localhost:8080/ai-platform/mcp/admin/tools
```
...
```bash
curl http://localhost:8080/ai-platform/mcp/admin/resources
```

### 4.4 프롬프트 관리

```bash
curl http://localhost:8080/ai-platform/mcp/admin/prompts
```
...
```bash
# 최근 50건 조회
curl http://localhost:8080/ai-platform/mcp/admin/audit
...
| 경로 | 메서드 | 설명 |
|------|--------|------|
| `/ai-platform/mcp` | POST | JSON-RPC 2.0 요청 수신 |
| `/ai-platform/mcp` | GET (SSE) | 서버 이벤트 스트림 (Accept: text/event-stream) |
| `/ai-platform/mcp/health` | GET | 서버 상태 (liveness/readiness) |
| `/ai-platform/mcp/admin/servers` | GET, POST | MCP 서버 인스턴스 관리 |
| `/ai-platform/mcp/admin/tools` | GET, PUT | 툴 카탈로그 및 차단 관리 |
| `/ai-platform/mcp/admin/resources` | GET | 리소스 카탈로그 조회 |
| `/ai-platform/mcp/admin/prompts` | GET | 프롬프트 카탈로그 조회 |
| `/ai-platform/mcp/admin/sessions` | GET | 활성 세션 목록 |
| `/ai-platform/mcp/admin/audit` | GET | 감사 로그 (쿼리 파라미터: limit, method) |
| `/ai-platform/mcp/admin/policies` | GET, PUT | 인증/인가/차단 정책 관리 |
...
- [ ] `/ai-platform/mcp/health` 200 OK 확인
