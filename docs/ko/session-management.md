...
기본 `InMemoryHttpSessionStore` 외에 `HttpSessionStore` SPI와
`ClusteredHttpSessionStore`가 추가되었다.

- sticky session 지원
- 비동기 세션 복제
- 직렬화 정책 (`STRICT`, `DROP_NON_SERIALIZABLE`)
- 세션 충돌 해소 (`LatestVersionSessionConflictResolver`)
- 절대 만료시각 기반 TTL 일관성

상세 사용법과 설정 방법은 [클러스터 세션 사용 및 설정 가이드](cluster-session-guide.md)를 참고한다.
